...
class LitDigestTests(unittest.TestCase):
    ...
    def test_update_preferences_from_zotero_merges_into_history(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            store = PaperStore(str(Path(td) / "papers.db"))
            paper = Paper("other", "title", "title", "summary", ["A B"], "http://a", "2025-01-01")
            store.upsert(paper, "other", 3.1)
            rows = store.top_papers(1)
            self.assertEqual(len(rows), 1)
            self.assertEqual(rows[0][1], "title")
            # authors are stored as JSON in column index 4
            self.assertEqual(json.loads(rows[0][4])[0], "A B")
