...
- 偏好支持：
  ...
  - 关键词自动去重归一，固定保留 200 个并按 20/180 分层权重（2/1）
...
- 总分范围：`0 ~ 60`
- 关键词分（上限 `30`）：
  - 基于 `title + summary` 与偏好关键词匹配强度计算
  - 使用慢饱和曲线，拉开中低分区分度，避免大量文章集中在高分
  - 对文本较短/摘要不完整的条目有 token 兜底分
- 作者分（不累加，取最高档）：
  - 命中作者权重 `1`：`20` 分
  - 命中作者权重 `2`：`30` 分
- 网页相关度条显示：
  - 前端进度条按 `50` 分封顶归一化显示（即分数大于 50 仍显示满格）
...
- `更新推荐`：执行完整抓取流程（arXiv + RSS）并更新数据库
- `更新排序（不抓取）`：不抓取新文章，只按当前 `preferences.json` 对 `papers.db` 现有文章重新打分排序，并同步刷新 `digest.md`
