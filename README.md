...
## 基于 Zotero 历史库 + 分类更新偏好

可以从 Zotero 导出（含 tags/collections）中自动识别偏好并**合并更新到现有** `preferences.json`：

```bash
python -m src.lit_digest \
  --update-preferences-from-zotero \
  --zotero-export zotero-export.json \
  --history preferences.json
```

说明：
- 会读取标题、摘要、作者、标签、分类（collections/folder/section）。
